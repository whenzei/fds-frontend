<template>
    <v-container>
        <h2 style="color: orange; padding: 50px 50px"> Manage Global Promotions</h2>

        <v-data-table :headers="promoHeaders" :items="promoList" class="elevation-1">
            <template v-slot:top>
                <v-toolbar color="black orange--text text--darken-2" flat>
                    <v-toolbar-title color="orange darken-2">Global Promotions</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-col cols="4" md="1">
                        <v-row justify="end">
...
<script>
    ...
    export default {
        ...
        methods: {
            async getPromos() {
                const res = await axios.get(`/manager/global-promos/`);
                let items = [];
            ...
            async insertIntoDatabase(item) {
                const params = { promo: item };
                const res = await axios.post(`/manager/add-global-promo/`, params);
                if (res.status == 200) {
            ...
            async updateDatabase(item) {
                console.log(item);
                const params = { promo:item };
                const res = await axios.post(`/manager/update-global-promo/`, params);
                if (res.status == 200) {
            ...
            async deleteFromDatabase(pid) {
                const res = await axios.post('/manager/delete-global-promo/' + pid);
                if (res.status == 200) {
